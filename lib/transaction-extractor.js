...
export class TransactionExtractor {
  ...
  static async extractFromFile(filePath, targetMonth) {
    ...
    for (const row of rows) {
      ...
      if (mapping.debit && mapping.credit) {
        ...
        // Only keep debits (outgoing money)
        if (debit > 0) amount = debit;
      } else if (mapping.amount) {
        const amt = CsvParser.parseAmount(row[mapping.amount]);
        // Negative amounts are outgoing
        if (amt < 0) amount = Math.abs(amt);
      }

      // Skip if no outgoing amount
    ...
    console.log(`  Extracted ${transactions.length} outgoing transactions`);
    return transactions;
  }
  ...
  static async extractFromDirectory(inputsDir, targetMonth) {
    ...
    console.log(`\nTotal outgoing transactions: ${allTransactions.length}`);
