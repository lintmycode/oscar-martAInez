  ...
  createSheet(workbook, name, rows, options = {}) {
    ...
    if (withTotal) {
      ...
      if (name === 'personal account') {
        ...
        styleSummaryRow(subtotalRow);

        const despesasRow = sheet.addRow({
          date: '',
          vendor: 'DESPESAS RECUPERADAS',
          amount: 0,
          invoice: '',
          notes: '',
        });
        styleSummaryRow(despesasRow);

        const totalRow = sheet.addRow({
          date: '',
          vendor: 'TOTAL',
          amount: { formula: `C${subtotalRow.number}-C${despesasRow.number}` },
          invoice: '',
          notes: '',
        });
